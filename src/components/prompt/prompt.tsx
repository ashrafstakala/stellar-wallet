...
export class Prompt {
  ...
  watchHandler(newValue: Prompter, oldValue: Prompter) {
    ...
    if (newValue.show) {
      ...
      if (newValue.options) this.input = this.input || `${newValue.options[0].code}:${newValue.options[0].issuer}`;
  ...
  componentDidLoad() {
    addEventListener('keyup', (e: KeyboardEvent) => {
      if (this.prompter.show) e.keyCode === 13 ? this.submit(e) : e.keyCode === 27 ? this.cancel(e) : null;
    });
  }
      ...
      <div class="prompt-wrapper">
        <div class="prompt">
          ...
          {this.prompter.options ? (
            <div class="select-wrapper">
              <select onInput={e => this.update(e)}>
                ...
                {this.prompter.options.map(option => (
                  <option value={`${option.code}:${option.issuer}`} selected={this.input === `${option.code}:${option.issuer}`}>
                    {option.code}
                  </option>
                ))}
              </select>
            ...
            <input type="text" placeholder={this.prompter.placeholder} value={this.input} onInput={e => this.update(e)}></input>
          )}
